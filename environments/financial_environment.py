...
class FinancialEnvironment(py_environment.PyEnvironment):
    def __init__(self, data, window_size):
        ...
        self.current_step = 0
        self._start_offset = window_size+1#np.random.randint(window_size+1, len(data)-1-maximum_steps)
    ...
    def _next_observation(self):
        obs = np.empty((self.window_size, 6), dtype=np.float32)
        for i in range(self.window_size):
            cur_idx = self.current_step - self.window_size + i
            candle = np.array([
                (self.df.loc[cur_idx, 'open']-self.df.loc[cur_idx-1, 'open'])/self.df.loc[cur_idx-1, 'open'],
                (self.df.loc[cur_idx, 'close']-self.df.loc[cur_idx-1, 'close'])/self.df.loc[cur_idx-1, 'close'],
                (self.df.loc[cur_idx, 'high']-self.df.loc[cur_idx-1, 'high'])/self.df.loc[cur_idx-1, 'high'],
                (self.df.loc[cur_idx, 'low']-self.df.loc[cur_idx-1, 'low'])/self.df.loc[cur_idx-1, 'low'],
                (self.df.loc[cur_idx, 'volume']-self.df.loc[cur_idx-1, 'volume'])/self.df.loc[cur_idx-1, 'volume'] if self.df.loc[cur_idx-1, 'volume'] else 0,
    ...
    def _reset(self):
        ...
        print()
        self._render_offset = self._start_offset
        self._start_offset = self.window_size+1#np.random.randint(window_size+1, len(self.df)-1)
        self.step_counter = 0
        self.current_step = self._start_offset
        self.steps_beyond_done = None
    ...
    def observation_spec(self):
        return tf_agents.specs.BoundedArraySpec(shape=(self.window_size, 6), dtype=np.float32, minimum=0, maximum=np.inf, name='observation')
...
def create_environment(data, window_size):
    train_py_env = FinancialEnvironment(data, window_size)
    eval_py_env = FinancialEnvironment(data, window_size)
